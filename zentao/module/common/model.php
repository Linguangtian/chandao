...
class commonModel extends model
{
    ...
    public   function  UrgentJob(){
        $this->app->UrgentJob=$this->dao->select('t1.*')->from(TABLE_TASK)->alias('t1')
            ...
            ->andWhere('t1.pri')->eq('1')
            ->count();
        $this->app->tomorrowJob=$this->dao->select('t1.*')->from(TABLE_TASK)->alias('t1')
            ->leftJoin(TABLE_USER)->alias('t2')->on('t1.assignedTo = t2.account')
            ->where('t1.assignedTo')->ne('')
            ->andWhere('t1.deleted')->eq(0)
            ->andWhere('t2.deleted')->eq(0)
     ...
     public static  function  printUrgentJob(){
        global $app;
