...
<div id='mainContent' class='main-row'>
  ...
  <div class="side-col">
    <div class="cell" id="taskLists">
      <?php foreach($taskGroup as $cacheId => $dateGroup):?>
      <ul class="task-list" data-month="<?php echo $cacheId;?>">
        <?php ksort($dateGroup);?>
        <?php foreach($dateGroup as $date => $tasks):?>
        <?php if(empty($startDate)) $startDate = $date;?>
        <li class="heading text-secondary" data-day="<?php echo $date;?>"><?php echo $date;?></li>
        <?php foreach($tasks as $task):?>
        <li><a class="iframe" href='<?php echo $task['url'];?>'><span class="status-<?php echo $task['status'];?> status"></span><?php echo $task['title'];?></a></li>
        <?php $events[] = $task;?>
        <?php endforeach;?>
        <?php endforeach;?>
      </ul>
...
$(function()
{
    ...
    var calendar = $calendar.calendar(
    {
        ...
        {
            var $event = $('<div title="' + event.title + '" data-id="' + (event.id || '') + '" class="event" title="' + (event.desc || '') + '"><span class="status-' + event.status + '">' + ' <span class="label label-dot"></span></span> <span class="title">' + event.title + '</span></div>');
            return $event;
        },
